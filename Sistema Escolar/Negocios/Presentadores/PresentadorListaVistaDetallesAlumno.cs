...
{
    public class PresentadorListaVistaDetallesAlumno : Presentador<List<VistaDetallesAlumno>>
    {
        public override List<VistaDetallesAlumno> Procesar(DataTable dataTable)
        {
            try
            {
                var alumnos = new List<VistaDetallesAlumno>();

                for (int i = 0; i < dataTable.Rows.Count; i++)
                {
                    DataRow row = dataTable.Rows[i];

                    alumnos.Add(new VistaDetallesAlumno()
                    {
